 ...
 *
 *
 * Каждая партия записывается в файл. Назание файла "game" + №(партии) ".txt". Формат файла: первые 4 строчки дополнительная информация для партии (первые 2 - кто и против кого
 * играет, последние 2 - растоновка позиций) далее идёт запись ходов партии. Ход записывается следующим образом:
 * ход белых, потом ход чёрных. Пример: e3:d4 e2:c4:a6(d3, b5). В скобках съеденные шашки
 * */
public class FileMaster{
    ...
    //записывает в файл  результат текущей партии
    ...
    private FileWriter createNewFile() throws IOException {
        ...
        new String();
        writeInfo.write(String.valueOf(gameNumber));
        writeInfo.write('\n');
        writeInfo.close();
        saveFilename = fileDirectory + game + ".txt";
        return new FileWriter(saveFilename);
    }
